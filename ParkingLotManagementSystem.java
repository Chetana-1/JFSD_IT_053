...
interface ParkingOperations{
    boolean parkVehicle(String vechileNumber);
...
abstract class ParkingSpot{
    ...
    for (ParkingSpot spotID : spots.values()) {
        if (spotID.isOccupied && spotID.getvehicleDetails().equals(vehicleNumber)) {
            spotID.isOccupied = false;
            spotID.vehicleDetails = "";
            System.out.println("Vehicle " + vehicleNumber + " has left from Spot " + spotID.getspotID());
            return true;
        }
    }
    ...
    for (ParkingSpot spotID : spots.values()) {
        if (spotID.isOccupied) {
            System.out.println(" Spot " + spotID.getspotID() + " - Vehicle: " + spotID.getvehicleDetails());
            empty = false;
        }
    }
    if (empty) {
